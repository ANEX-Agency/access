<?php
  ...
  if(!defined('RESIZE_SMALLER_THAN')) {
    define('RESIZE_SMALLER_THAN', 5242880); // resize images smaller than 5MB
  } // if
  ...
  define('APPLICATION_PATH', ROOT . '/system');
  define('DEVELOPMENT_PATH', ROOT . '/development');
  
  define('VAR_PATH', ENVIRONMENT_PATH . '/var');
  define('WORK_PATH', VAR_PATH . '/tmp');

  if(!defined('UPLOAD_PATH')) {
    define('UPLOAD_PATH', VAR_PATH . '/uploads/files');
  } // if

  //define('CUSTOM_PATH', ENVIRONMENT_PATH . '/custom');
  define('LOCALIZATION_PATH', VAR_PATH . '/languages');
  define('THUMBNAILS_PATH', VAR_PATH . '/uploads/images');
  
  if (!defined('PROJECT_EXPORT_PATH')) {
    define('PROJECT_EXPORT_PATH', VAR_PATH . '/exports');
  } // if
