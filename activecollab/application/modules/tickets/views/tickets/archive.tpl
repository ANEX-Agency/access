                    ...
                    {if $pagination->getLastPage() > 1}
                        {if isset($active_category) && instance_of($active_category, 'Category') && $active_category->isLoaded()}
                            <div class="pagination">
                            	{pagination pager=$pagination}{assemble route=project_tickets_archive project_id=$active_project->getId() page='-PAGE-' category_id=$active_category->getId()}{/pagination}
                            </div>
                        {else}
                            <div class="pagination">
                            	{pagination pager=$pagination}{assemble route=project_tickets_archive project_id=$active_project->getId() page='-PAGE-'}{/pagination}
                            </div>
                        {/if}
                    {/if}
            ...
            {if is_foreachable($categories)}
                {foreach from=$categories item=category}
                    <li category_id="{$category->getId()}" {if $active_category->isLoaded() && $active_category->getId() == $category->getId()}class="selected"{/if}>
                    	<a href="{assemble route=project_tickets_archive project_id=$active_project->getId() category_id=$category->getId()}">
