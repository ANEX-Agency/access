<div class="select_asignees_inline_widget" id="{$_select_assignees_id}">
    ...
    {foreach from=$_select_assignees_users key=company item=users}
        ...
        <div class="user_group">
            ...
            <label for="{$_select_assignees_id}_company_{$company|clean}">
                <!--<input type="checkbox" name="" value="" id="{$_select_assignees_id}_company_{$company|clean}}" />-->
                <span>{lang company_name=$company}:company_name{/lang}</span>
            </label>
                        ...
                        {foreach from=$users item=user name=users_loop}
                            ...
                            <td>
                                <span class="company_user">
                                    ...
                                    <input type="checkbox" name="{$_select_assignees_name}[]" value="{$user.id}" id="{$_select_assignees_id}_user_{$user.id}" {if in_array($user.id, $_select_assignees_assigned)}checked="checked"{/if} />
